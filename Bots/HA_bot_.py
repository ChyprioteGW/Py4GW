...
class Routines:
    ...
    def TravelToGTOB():
        ...
        if BotVariables.ha_quest_button_state == False or Quest.GetActiveQuest() == ha_quest_id:
            next_step = FSMVariables.fsm.get_next_step_name()
            if next_step:
                FSMVariables.fsm.jump_to_state_by_name(next_step)
            return
    ...
    def GetQuest():
        ...
        if not Agent.IsMoving(Player.GetAgentID()) and BotVariables.action_timer.IsRunning() and BotVariables.action_timer.HasElapsed(8000):
...
FSMVariables.fsm.AddState(name="Waiting to Resign", execute_fn=lambda: Routines.WaitForResign(), exit_condition=lambda: Map.GetMapID() == heroes_ascent, transition_delay_ms=5000, run_once=False) 
FSMVariables.fsm.AddState(name="Traveling to HA", execute_fn=lambda: Routines.TravelToHA(), exit_condition=lambda: Map.GetMapID() == heroes_ascent, transition_delay_ms=5000, run_once=True)

FSMVariables.fsm.AddState(name="Traveling to GtoB", execute_fn=lambda: Routines.TravelToGTOB(), exit_condition=lambda: Map.GetMapID() == gtob, transition_delay_ms=2000, run_once=True)
FSMVariables.fsm.AddState(name="Getting Quest", execute_fn=lambda: Routines.GetQuest(), exit_condition=lambda: Map.GetMapID() == gtob and Quest.GetActiveQuest() == ha_quest_id, transition_delay_ms=2000, run_once=False)
FSMVariables.fsm.AddState(name="Accepting Quest Reward", execute_fn=lambda: Routines.CompleteQuest(), exit_condition=lambda: Map.GetMapID() == gtob and not Quest.GetActiveQuest() == ha_quest_id, transition_delay_ms=2000, run_once=False)
...
FSMVariables.fsm.AddState(name="Entering HA Mission", execute_fn=lambda: Routines.EnterMission(), exit_condition=lambda: Map.GetMapID() == heroes_ascent and Map.IsExplorable(), transition_delay_ms=1000, run_once=True)

FSMVariables.fsm.AddState(name="Zaishen", execute_fn=lambda: Routines.Zaishen(), exit_condition=lambda: Map.GetMapID() == underworld or (Map.GetMapID() == heroes_ascent and not Map.IsExplorable()), transition_delay_ms=15000, run_once=False)
FSMVariables.fsm.AddState(name="Underworld", execute_fn=lambda: Routines.Underworld(), exit_condition=lambda: Map.GetMapID() != underworld, transition_delay_ms=5000, run_once=False)
FSMVariables.fsm.AddState(name="Fetid River", execute_fn=lambda: Routines.Fetid_River(), exit_condition=lambda: Map.GetMapID() != fetid_river, transition_delay_ms=5000, run_once=False)
FSMVariables.fsm.AddState(name="Burial Mounds", execute_fn=lambda: Routines.Burial_Mounds(), exit_condition=lambda: Map.GetMapID() != burial_mounds, transition_delay_ms=5000, run_once=False)
