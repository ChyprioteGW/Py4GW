...
class Routines:
    ...
    def PracticeRotation():
        ...
        if target and target != 0 and Agent.IsAlive(target) and not Map.IsMapLoading() and AgentArray.Filter.ByDistance(alive_enemies, Player.GetXY(),
                                                                                                                        Range.Spellcast.value - 200) and Map.IsMapReady():
...
FSMVariables.fsm.AddState(name="Waiting to Resign", execute_fn=lambda: Routines.WaitForResign(), exit_condition=lambda: Map.GetMapID() == heroes_ascent and not Map.IsMapLoading() and Map.IsMapReady(),
                          transition_delay_ms=5000, run_once=False)
FSMVariables.fsm.AddState(name="Travelling to HA", execute_fn=lambda: Routines.TravelToHA(), exit_condition=lambda: Map.GetMapID() == heroes_ascent and not Map.IsMapLoading() and Map.IsMapReady(),
                          transition_delay_ms=5000, run_once=True)
FSMVariables.fsm.AddState(name="Checking Zaishen Quest State", execute_fn=lambda: Routines.TravelToGTOB(),
                          exit_condition=lambda: Map.GetMapID() == gtob and not Map.IsMapLoading() and Map.IsMapReady(), transition_delay_ms=2000, run_once=True)
FSMVariables.fsm.AddState(name="Getting Quest", execute_fn=lambda: Routines.GetQuest(),
                          exit_condition=lambda: Map.GetMapID() == gtob and not Map.IsMapLoading() and Map.IsMapReady() and Quest.GetActiveQuest() == ha_quest_id, transition_delay_ms=2000,
                          run_once=False)
FSMVariables.fsm.AddState(name="Accepting Quest Reward", execute_fn=lambda: Routines.CompleteQuest(),
                          exit_condition=lambda: Map.GetMapID() == gtob and not Map.IsMapLoading() and Map.IsMapReady() and not Quest.GetActiveQuest() == ha_quest_id, transition_delay_ms=2000,
                          run_once=False)
...
FSMVariables.fsm.AddState(name="Buying from Tolkano", execute_fn=lambda: Routines.SendDialogs(), exit_condition=lambda: Player.GetBalthazarData()[0] < 5000 or BotVariables.interaction_stage > 3,
                          transition_delay_ms=5000, run_once=False, )
FSMVariables.fsm.AddState(name="Entering HA Mission", execute_fn=lambda: Routines.EnterMission(),
                          exit_condition=lambda: Map.GetMapID() == heroes_ascent and Map.IsExplorable() and not Map.IsMapLoading() and Map.IsMapReady(), transition_delay_ms=1000, run_once=True)
FSMVariables.fsm.AddState(name="Zaishen", execute_fn=lambda: Routines.Zaishen(), exit_condition=lambda: Map.GetMapID() == underworld and not Map.IsMapLoading() and Map.IsMapReady(),
                          transition_delay_ms=15000, run_once=False)
FSMVariables.fsm.AddState(name="Underworld", execute_fn=lambda: Routines.Underworld(), exit_condition=lambda: Map.GetMapID() != underworld and not Map.IsMapLoading() and Map.IsMapReady(),
                          transition_delay_ms=5000, run_once=False)
FSMVariables.fsm.AddState(name="Fetid River", execute_fn=lambda: Routines.Fetid_River(), exit_condition=lambda: Map.GetMapID() != fetid_river and not Map.IsMapLoading() and Map.IsMapReady(),
                          transition_delay_ms=5000, run_once=False)
FSMVariables.fsm.AddState(name="Burial Mounds", execute_fn=lambda: Routines.Burial_Mounds(), exit_condition=lambda: Map.GetMapID() != burial_mounds and not Map.IsMapLoading() and Map.IsMapReady(),
                          transition_delay_ms=5000, run_once=False)
